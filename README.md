...
Publieke blocklist voor de **[Wortell Browser Protect](https://github.com/wortell/browser-protect)** browser-extensie. De extensie haalt `blocklist.txt` elk uur op en blokkeert pagina's waarvan de **hostname** op de lijst staat — een snelle manier om een bekende phishing-URL voor alle klanten te weren.

## Hoe het werkt

- De extensie fetcht:
  `https://raw.githubusercontent.com/wortell/browser-protect-feed/main/blocklist.txt`
- Matching is op **host/domein-niveau** (niet op pad). Verversen gebeurt per uur; bij een storing **fail-open** (de laatst opgehaalde lijst blijft gelden, de rest van de bescherming werkt door).
...
- Commit naar `main` → clients pikken het binnen ~1 uur op.
